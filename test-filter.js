...
async function test() {
  var { data, error } = await sb.from('comments').select('*').limit(10);
  if (error) { console.error(error); return; }
  
  data.forEach(c => {
    var content = (c.content || '').trim().toUpperCase();
    var isFiltered = content.startsWith('[BANNI]') || content.startsWith('[SUPPRIMÉ]');
    console.log(`Content: ${c.content} -> Filtered: ${isFiltered}`);
  });
}
